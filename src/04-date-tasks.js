...
function isLeapYear(date) {
  if (((date.getFullYear() % 100 !== 0) && (date.getFullYear() % 4 === 0))
...
function angleBetweenClockHands(date) {
  ...
  const hoursOnDegrees = 0.5 * (60 * hour + date.getUTCMinutes());
  const minutes = 6 * date.getUTCMinutes();
  // eslint-disable-next-line max-len
  const difference = hoursOnDegrees - minutes > 180 ? hoursOnDegrees - minutes - 180 : hoursOnDegrees - minutes;
